...
export function manageVoiceRoutine(input, deps) {
    const now = deps.now ?? Date.now;
    const targetId = input.targetId ?? "";
    const routineName = String(input.name ?? "").trim();
    const resolveRoutine = () => {
        if (!routineName)
            throw new Error("name the routine to manage");
        const matches = deps.routines.listRoutines().filter((routine) => normalized(routine.name) === normalized(routineName));
        if (!matches.length)
            throw new Error(`no routine named ${routineName}`);
        if (matches.length > 1)
            throw new Error(`several routines are named ${routineName}; rename one in the calendar first`);
        return matches[0];
    };
    ...
    if (input.action === "create") {
        ...
        if (!bot)
            throw new Error("the selected agent does not exist");
        if (!routineName)
            throw new Error("give the routine a name");
        if (!input.prompt?.trim())
            throw new Error("say what the routine must do");
        const schedule = input.scheduleType === "once"
            ? (() => {
                const at = Date.parse(input.at ?? "");
                if (!Number.isFinite(at) || at <= now())
                    throw new Error("give a future date and time with a timezone");
                return { type: "once", at };
            })()
            : input.scheduleType === "daily"
                ? { type: "daily", time: input.time ?? "", weekdays: input.weekdays ?? [0, 1, 2, 3, 4, 5, 6] }
    ...
    if (!run)
        throw new Error("the routine could not be started");
