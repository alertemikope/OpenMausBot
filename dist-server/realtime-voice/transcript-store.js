...
export class VoiceTranscriptStore {
    ...
    get(sessionId) {
        try {
            return JSON.parse(readFileSync(join(this.directory, `${safeId(sessionId)}.json`), "utf8"));
        }
        catch {
            return undefined;
        }
    ...
    list(limit = 20) {
        ...
        return files
            .map((file) => {
            try {
                return JSON.parse(readFileSync(join(this.directory, file), "utf8"));
            }
            catch {
                return undefined;
            }
        })
            .filter((value) => Boolean(value?.sessionId && Array.isArray(value.entries)))
            .sort((left, right) => right.startedAt - left.startedAt)
            .slice(0, Math.max(0, Math.min(limit, 100)));
    }
    recentContext(limit = 5) {
