...
import { liveAuthHeaders } from "./openai-live-wire.js";
...
class NodeLiveSidebandSocket extends EventEmitter {
    readyState = OPEN;
    ...
    send(payload) {
        if (this.readyState !== OPEN)
            throw new Error("GPT-Live sideband is closed");
    ...
    consume(chunk) {
        ...
        while (this.buffer.length >= 2) {
            ...
            if (second & 0x80)
                return this.fail(new Error("GPT-Live sideband returned a masked server frame"));
            let offset = 2;
            ...
            if (length === 126) {
                ...
                if (large > BigInt(MAX_PAYLOAD))
                    return this.fail(new Error("GPT-Live sideband frame is too large"));
                length = Number(large);
                offset = 10;
            }
            ...
            if (opcode !== 0x0 && opcode !== 0x1)
                return this.fail(new Error("GPT-Live sideband returned unsupported binary data"));
            ...
            if (message.length > MAX_PAYLOAD)
                return this.fail(new Error("GPT-Live sideband message is too large"));
...
export async function connectLiveSideband(params) {
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
        try {
            ...
            if (attempt < 4)
                await wait(200 * 2 ** attempt, params.signal);
        }
    }
    throw lastError;
}
