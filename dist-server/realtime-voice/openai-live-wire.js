...
import { LIVE_MODEL, LIVE_VOICES } from "./contracts.js";
...
function boundedText(value, max = MAX_TRANSCRIPT_CHARS) {
    return typeof value === "string" && value.length <= max ? value : undefined;
}
export function resolveLiveVoice(value) {
    const normalized = typeof value === "string" ? value.trim().toLowerCase() : "";
    return LIVE_VOICES.includes(normalized) ? normalized : "marin";
}
export function buildLiveSession(params) {
    if ((params.model ?? LIVE_MODEL) !== LIVE_MODEL)
    ...
    return {
        ...
        ...(initialText
            ? {
                initial_items: [
                    { type: "message", role: "user", content: [{ type: "input_text", text: initialText }] },
                ],
            }
            : {}),
    };
...
export function resolveChatGptIdentity(accessToken) {
    ...
    if (!accountId)
        throw new Error("ChatGPT access token is missing chatgpt-account-id");
...
export function liveAuthHeaders(auth, ids) {
    return {
...
function multipartBody(offerSdp, session) {
    const sessionJson = JSON.stringify(session);
    let boundary;
    do
        boundary = `openmaus-quicksilver-${randomBytes(18).toString("hex")}`;
...
async function readBounded(response, maxBytes) {
    ...
    if (bytes.byteLength > maxBytes)
        throw new Error("GPT-Live response exceeded the size limit");
...
export async function createLiveCall(params) {
    ...
    if (response.status === 403) {
        ...
        if (!answerSdp.trim())
            throw new Error("ChatGPT realtime fallback returned an empty SDP answer");
    ...
    if (!answerSdp.trim())
        throw new Error("GPT-Live returned an empty SDP answer");
    const location = response.headers.get("location");
    const headerId = response.headers.get("openai-session-id")?.trim() ?? "";
    let callId = "";
    try {
        callId = new URL(location ?? "", LIVE_URL).pathname.split("/").filter(Boolean).find((value) => CALL_ID.test(value)) ?? "";
    }
    catch { }
    if (!CALL_ID.test(callId) && CALL_ID.test(headerId))
        callId = headerId;
    if (!CALL_ID.test(callId))
        throw new Error("GPT-Live response is missing a valid call id");
    return { kind: "gpt-live", model: LIVE_MODEL, answerSdp, sidebandUrl: `wss://api.openai.com/v1/live/${callId}` };
}
export function parseLiveEvent(payload) {
    let decoded;
    try {
    ...
    if (type === "output_audio.delta" || type === "session.updated")
        return { kind: "ignored", eventType: type };
    return { kind: "unknown", eventType: type };
}
export function chunkDelegationText(text, maxBytes = 500) {
    const chunks = [];
    let current = "";
    let bytes = 0;
