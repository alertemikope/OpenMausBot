...
import {
  ...
  type WorkItemCreate,
  type WorkItemState,
  type WorkRuntimeStatus,
} from "./contracts.ts";

type WorkFile = { version: 1; items: WorkItem[] };

export type WorkRegistryOptions = {
  file?: string;
  now?: () => number;
...
const activeStates = new Set<WorkItemState>(ACTIVE_WORK_STATES);
const terminalStates = new Set<WorkItemState>(TERMINAL_WORK_STATES);

function clone(item: WorkItem): WorkItem {
  return { ...item, ...(item.denials ? { denials: [...item.denials] } : {}) };
}

function routineState(run: RoutineRun): WorkItemState {
  switch (run.status) {
    case "queued":
      return "queued";
    case "running":
...
export class WorkRegistry {
  ...
  constructor(options: WorkRegistryOptions = {}) {
    ...
    try {
      const disk = JSON.parse(readFileSync(this.file, "utf8")) as Partial<WorkFile>;
      this.items = Array.isArray(disk.items) ? disk.items.filter((item): item is WorkItem => this.validItem(item)) : [];
    ...
    for (const item of this.items) {
      if (!["running", "waiting_approval", "waiting_input"].includes(item.state)) continue;
  ...
  create(input: WorkItemCreate): WorkItem {
    ...
    if (!objective) throw new Error("Work objective is required");
    const targetBotId = String(input.targetBotId ?? "").trim();
    if (!targetBotId) throw new Error("Work target is required");
    const item: WorkItem = {
      id,
      origin: input.origin,
      targetBotId,
  ...
  list(options: { targetBotId?: string; active?: boolean; limit?: number } = {}): WorkItem[] {
  ...
  queued(origin?: WorkItem["origin"]): WorkItem[] {
    return this.items
  ...
  activeForTarget(targetBotId: string): WorkItem | undefined {
  ...
  runtimeStatus(options: { staleAfterMs?: number; cancellationAfterMs?: number } = {}): WorkRuntimeStatus {
    const at = this.now();
  ...
  claim(id: string): boolean {
    ...
    const at = this.now();
    Object.assign(item, { state: "running" as const, progress: "Dispatching to agent", startedAt: at, updatedAt: at });
    this.persistAndEmit(item);
    return true;
  }

  bindThread(id: string, threadId: string): WorkItem | undefined {
    const item = this.mutable(id);
    if (!item || terminalStates.has(item.state)) return item ? clone(item) : undefined;
  ...
  bindTurn(id: string, turnId: string, providerInstanceId?: string): WorkItem | undefined {
    const item = this.mutable(id);
    if (!item || terminalStates.has(item.state)) return item ? clone(item) : undefined;
    if (item.turnId && item.turnId !== turnId) return clone(item);
    item.turnId = turnId;
    if (providerInstanceId) item.providerInstanceId = providerInstanceId;
  ...
  failDispatch(id: string, message: string): WorkItem | undefined {
    const item = this.mutable(id);
    if (!item || terminalStates.has(item.state)) return item ? clone(item) : undefined;
    const at = this.now();
    Object.assign(item, {
  ...
  requestCancel(id: string): WorkItem | undefined {
    const item = this.mutable(id);
    if (!item || !activeStates.has(item.state)) return item ? clone(item) : undefined;
    const at = this.now();
  ...
  cancelRequestFailed(id: string, message: string): WorkItem | undefined {
    const item = this.mutable(id);
    if (!item || terminalStates.has(item.state)) return item ? clone(item) : undefined;
  ...
  cancelQueued(id: string, reason = "Cancelled before dispatch"): boolean {
    const item = this.mutable(id);
    if (!item || item.state !== "queued") return false;
    const at = this.now();
    Object.assign(item, {
  ...
  markSeen(id: string): WorkItem | undefined {
    const item = this.mutable(id);
  ...
  handleRuntimeEvent(event: RuntimeEvent): void {
    ...
    if (!item) return;
    const at = this.now();
    if (event.providerInstanceId) item.providerInstanceId = event.providerInstanceId;
    switch (event.type) {
      case "turn.started":
      ...
      case "request.resolved":
        if (event.requestId && item.requestId && event.requestId !== item.requestId) return;
        item.state = "running";
        item.requestId = undefined;
        item.requestSummary = undefined;
  ...
  upsertRoutineRun(run: RoutineRun): WorkItem {
    ...
    if (!(item.state === "waiting_approval" && nextState === "waiting_input")) item.state = nextState;
  ...
  private ownerForEvent(event: RuntimeEvent): WorkItem | undefined {
    if (event.turnId) {
  ...
  private mutable(id: string): WorkItem | undefined {
    return this.items.find((item) => item.id === id);
  }

  private validItem(value: unknown): value is WorkItem {
    ...
    return validShape && (activeStates.has(item.state as WorkItemState) || terminalStates.has(item.state as WorkItemState));
  }
  ...
  private persistAndEmit(item: WorkItem): void {
    this.save();
  ...
  private save(): void {
    mkdirSync(dirname(this.file), { recursive: true, mode: 0o700 });
    writeFileAtomic(this.file, JSON.stringify({ version: 1, items: this.items } satisfies WorkFile, null, 2), 0o600);
  }
}
