...
export type PiMemoryIntegration = { command: string; args: string[]; env: Record<string, string> };

const agentDir = () => process.env.PI_CODING_AGENT_DIR || join(homedir(), ".pi", "agent");

function validBridge(path: unknown): string | null {
...
export function resolvePiMemoryMcp(
  override = process.env.OPENMAUSBOT_PI_MEMORY_MCP,
  piAgentDir = agentDir(),
): string | null {
  ...
  if (override !== undefined) return validBridge(override);

  try {
    const settings = JSON.parse(readFileSync(join(piAgentDir, "settings.json"), "utf8")) as { packages?: unknown };
...
export function piMemoryIntegration(): PiMemoryIntegration | null {
