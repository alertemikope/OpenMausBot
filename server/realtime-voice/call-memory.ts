...
import type { VoiceTranscript, VoiceTranscriptEntry, VoiceTranscriptStore } from "./transcript-store.ts";
...
export type MemoryCandidateStatus = "pending" | "syncing" | "kept" | "ignored" | "forgotten";
...
export type FollowUpCandidate = CallInsight & {
...
export type CallMemoryReview = {
  ...
  updatedAt: number;
  workingState: CallWorkingState;
  memoryCandidates: CallMemoryCandidate[];
  followUpCandidates: FollowUpCandidate[];
  error?: string;
};

export type CanonicalMemoryPort = {
  write(input: { content: string; type: "fact" | "preference"; source: string }): Promise<{ id: string }>;
  correct(input: { id: string; content: string; reason: string }): Promise<{ id: string }>;
  forget(id: string): Promise<void>;
};

export type CallMemoryProcessor = {
  process(sessionId: string): Promise<VoiceTranscript | undefined>;
  review(input: {
    sessionId: string;
    candidateId: string;
...
const EMPTY_WORKING_STATE: CallWorkingState = {
  decisions: [],
  commitments: [],
  openQuestions: [],
  deadlines: [],
...
function insightId(kind: string, quote: string, at: number): string {
  return createHash("sha256").update(`${kind}\0${at}\0${quote.toLocaleLowerCase()}`).digest("hex").slice(0, 20);
}

function insight(kind: string, quote: string, entry: VoiceTranscriptEntry): CallInsight {
  return { id: insightId(kind, quote, entry.at), text: quote, sourceQuote: quote, at: entry.at };
}

function segments(entry: VoiceTranscriptEntry): string[] {
...
function pushUnique<T extends CallInsight>(target: T[], value: T): void {
  if (!target.some((item) => item.text.toLocaleLowerCase() === value.text.toLocaleLowerCase())) target.push(value);
}

export function pendingCallMemoryReview(now = Date.now()): CallMemoryReview {
  return {
    status: "pending",
    updatedAt: now,
...
export function extractCallMemory(entries: VoiceTranscriptEntry[], now = Date.now()): CallMemoryReview {
  const workingState: CallWorkingState = { decisions: [], commitments: [], openQuestions: [], deadlines: [] };
  const memoryCandidates: CallMemoryCandidate[] = [];
  const followUpCandidates: FollowUpCandidate[] = [];

  for (const entry of entries.slice(-200)) {
    for (const quote of segments(entry)) {
      if (entry.role !== "user") continue;
      if (decisionPattern.test(quote)) pushUnique(workingState.decisions, insight("decision", quote, entry));
      ...
      if (questionPattern.test(quote)) pushUnique(workingState.openQuestions, insight("question", quote, entry));
      if (deadlinePattern.test(quote)) pushUnique(workingState.deadlines, insight("deadline", quote, entry));

      if (ambiguityPattern.test(quote) || sensitivePattern.test(quote)) continue;
...
export class CallMemoryService implements CallMemoryProcessor {
  private readonly transcripts: VoiceTranscriptStore;
  private readonly memory: CanonicalMemoryPort;
  private readonly now: () => number;

  constructor(transcripts: VoiceTranscriptStore, memory: CanonicalMemoryPort, now: () => number = Date.now) {
    this.transcripts = transcripts;
    this.memory = memory;
    this.now = now;
  }

  async process(sessionId: string): Promise<VoiceTranscript | undefined> {
  ...
  async review(input: {
    ...
    if (!call?.review) throw Object.assign(new Error("no extracted call memory"), { status: 404 });
    const candidate = call.review.memoryCandidates.find((item) => item.id === input.candidateId);
    if (!candidate) throw Object.assign(new Error("no such memory candidate"), { status: 404 });
