...
import { LIVE_MODEL, LIVE_VOICES, type LiveVoice, type OAuthAccess } from "./contracts.ts";
...
export type LiveInboundEvent =
  ...
  | { kind: "delegation"; id: string; prompt: string; mode?: "task" | "status" | "cancel" | "steer" | "followup" }
    ...
    if (name === "agent_consult" && id && args) {
      try {
        ...
        const mode = candidateMode === "task" || candidateMode === "status" || candidateMode === "cancel" || candidateMode === "steer" || candidateMode === "followup"
          ? candidateMode
          : undefined;
        if (prompt) return { kind: "delegation", id, prompt, ...(mode ? { mode } : {}) };
      } catch {}
    }
