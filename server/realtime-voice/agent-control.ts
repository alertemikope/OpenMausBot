import type { AgentControlMode } from "./contracts.ts";

export type AgentControlIntent = { mode: AgentControlMode; text: string };
...
export function classifyAgentControl(input: string): AgentControlIntent | null {
  const text = input.trim();
  if (!text || text.length > 2_000) return null;
  const marked = /^\[OPENMAUS_CONTROL:(status|cancel|steer|followup)\]\s*([\s\S]*)$/iu.exec(text);
  if (marked) {
    const mode = marked[1]!.toLowerCase() as AgentControlMode;
    const markedText = marked[2]!.trim();
    return markedText ? { mode, text: markedText } : null;
  }
  if (STATUS.test(text)) return { mode: "status", text };
  if (CANCEL.test(text)) return { mode: "cancel", text };
  const followup = FOLLOWUP.exec(text);
  if (followup?.[1]?.trim()) return { mode: "followup", text: followup[1].trim() };
  if (STEER.test(text)) return { mode: "steer", text };
  return null;
}
