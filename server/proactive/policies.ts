import type { ProactiveChannel, ProactivePolicy } from "./contracts.ts";
...
export function inQuietHours(policy: ProactivePolicy, at: number): boolean {
...
): { channels: ProactiveChannel[]; reason: string } {
  if (!policy.enabled) return { channels: [], reason: "Proactive notifications are disabled" };
  const quiet = inQuietHours(policy, at);
  const atLimit = interruptionsLastHour >= policy.maxInterruptionsPerHour;
  const channels = (["dock", ...policy.channels] as ProactiveChannel[]).filter((channel, index, all) => {
    if (all.indexOf(channel) !== index) return false;
    if (channel === "dock") return true;
    if (channel === "voice" && !voiceAvailable) return false;
    return !quiet && !atLimit;
  });
  if (!channels.length) return { channels, reason: quiet ? "Suppressed by quiet hours" : atLimit ? "Hourly interruption limit reached" : "No delivery channel enabled" };
  if (quiet) return { channels, reason: "Delivered silently to Mission Control during quiet hours" };
  if (atLimit) return { channels, reason: "Delivered silently to Mission Control after the hourly interruption limit" };
...
export function normalizePolicy(value: Partial<ProactivePolicy>): ProactivePolicy {
  const channels = Array.isArray(value.channels)
    ? [...new Set(value.channels.filter((channel): channel is ProactiveChannel => ["dock", "system", "voice"].includes(channel)))]
  ...
  for (const source of Object.keys(sources) as Array<keyof typeof sources>) {
